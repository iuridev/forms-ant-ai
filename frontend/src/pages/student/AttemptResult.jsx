...
import { CheckCircleOutlined, CloseCircleOutlined, ArrowLeftOutlined, WarningOutlined } from '@ant-design/icons';
    ...
    <div style={{ maxWidth: 720, margin: '32px auto', padding: '0 16px' }}>
      ...
      <Card style={{ textAlign: 'center', marginBottom: 16, borderRadius: 12 }}>
        ...
        <Progress
          ...
          format={() => <><Text style={{ fontSize: 32, fontWeight: 700 }}>{pct}%</Text><br /><Text type="secondary" style={{ fontSize: 13 }}>{attempt.score?.toFixed(1)} / {attempt.maxScore?.toFixed(1)} pts</Text></>}
      ...
      {attempt.violations?.length > 0 && (
        <Alert
          ...
          message={`${attempt.violations.length} violação(ões) registrada(s) durante esta prova`}
      ...
      <Card title="Gabarito — suas respostas" style={{ borderRadius: 12 }}>
        {attempt.answers?.map((a, idx) => (
          <div key={a.id} style={{ marginBottom: 16, padding: '12px 16px', borderRadius: 8, background: a.isCorrect ? '#f6ffed' : '#fff2f0', border: `1px solid ${a.isCorrect ? '#b7eb8f' : '#ffccc7'}` }}>
            <Space>
              {a.isCorrect ? <CheckCircleOutlined style={{ color: '#52c41a', fontSize: 18 }} /> : <CloseCircleOutlined style={{ color: '#ff4d4f', fontSize: 18 }} />}
